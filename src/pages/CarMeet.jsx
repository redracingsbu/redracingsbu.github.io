...
const MEET_WINNERS = [
  { 
    rank: "1", suffix: "st", name: "Amann", car: "2019 BMW M550i", 
    image: FirstPlaceImg, 
    highRes: FirstPlaceImg 
  },
  { 
    rank: "2", suffix: "nd", name: "Ricky", car: "2009 Toyota Camry", 
    image: SecondPlaceImg,
    highRes: SecondPlaceImg
  },
  { 
    rank: "3", suffix: "rd", name: "Ash", car: "1987 Nissan Datsun 200SX S12", 
    image: ThirdPlaceImg,
...
const GALLERY_IMAGES = Array.from({ length: 20 }, (_, i) => ({
  ...
  highRes: new URL(`../assets/carmeet/highres/${i + 1}hr.webp`, import.meta.url).href
}));
    ...
    <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center backdrop-blur-sm" onClick={onClose}>
      <div className="absolute top-0 left-0 w-full p-4 flex justify-between items-center text-white/80 z-50">
        ...
        <div className="flex gap-6">
           <a href={images[currentIndex].highRes} download onClick={(e) => e.stopPropagation()} className="hover:text-white transition-colors">
      ...
      <button onClick={prevImage} className="absolute left-4 top-1/2 -translate-y-1/2 p-4 text-white/50 hover:text-white transition-colors hidden lg:block z-50">
      ...
      <button onClick={nextImage} className="absolute right-4 top-1/2 -translate-y-1/2 p-4 text-white/50 hover:text-white transition-colors hidden lg:block z-50">
  ...
  return (
    <PageLayout wrapperClassName="rr-page" mainClassName="flex-1 flex flex-col">
      <div className="relative w-full h-[60vh] lg:h-[75vh] -mt-24 lg:-mt-24">
        <img src={CarMeetMain} alt="Campus Car Meet Group Photo" className="w-full h-full object-cover object-center" loading="eager" />
        <div className="absolute inset-0 bg-black/30"></div>
        <div className="absolute inset-0 flex justify-center pt-30">
          <h1 className="rr-h1 tracking-wide px-4 drop-shadow-lg text-center">Campus Car Meet</h1>
        </div>
      </div>
      ...
      <div className="bg-black text-white w-full pb-16 px-6 flex flex-col items-center">
        ...
        <h2 className="text-3xl lg:text-5xl font-bold mb-16 text-center">Fall 2025 Meet Winners!</h2>
          ...
          {MEET_WINNERS.map((winner, index) => (
            ...
            >
              <h3 className="text-3xl lg:text-4xl font-bold italic tracking-wide">
                {winner.rank}<sup className="text-2xl lg:text-3xl not-italic -top-2 relative">{winner.suffix}</sup> Place
              </h3>
              <div className="w-full aspect-video overflow-hidden rounded-sm shadow-lg">
                <img src={winner.image} alt="Winner Car" className="w-full h-full object-cover" />
              </div>
              <div className="space-y-1">
                <p className="text-xl lg:text-2xl font-medium text-white">{winner.name}</p>
                <p className="text-lg lg:text-xl text-gray-300 font-light">{winner.car}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="bg-black text-white w-full pb-6 px-4 flex flex-col items-center">
        <h2 className="text-4xl lg:text-6xl font-bold mb-12 text-center">Gallery</h2>
